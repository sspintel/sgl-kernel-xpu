...
@pytest.mark.parametrize(
    "dim", [128, 256, 512, 768, 2048, 4096, 11008, 16384, 25600, 28672]
)
...
def test_fused_silu_mul(dim, batch_size, seq_len):
    ...
    y_xpu = sgl_kernel.silu_and_mul(x_xpu)
    torch.testing.assert_close(y_ref, y_xpu.to("cpu"), rtol=1e-3, atol=1e-3)
...
def test_fused_gelu_tanh_mul(dim, batch_size, seq_len):
    ...
    x_xpu = x.clone().to(device)
    y_xpu = sgl_kernel.gelu_tanh_and_mul(x_xpu)
    torch.testing.assert_close(y_ref, y_xpu.to("cpu"), rtol=1e-3, atol=1e-3)
...
def test_fused_gelu_mul(dim, batch_size, seq_len):
    ...
    x_xpu = x.clone().to(device)
    y_xpu = sgl_kernel.gelu_and_mul(x_xpu)
    torch.testing.assert_close(y_ref, y_xpu.to("cpu"), rtol=1e-3, atol=1e-3)
